...
export class AIToolsService {
  constructor(@InjectRepository(AITool) private repo: Repository<AITool>) {}
  ...
  async findOne(id: string, orgId: string) {
    ...
    if (!item) throw new NotFoundException(`AI Tool ${id} not found`)
    return item
  }

  create(body: Partial<AITool>, orgId: string) {
    return this.repo.save(this.repo.create({ ...body, organization_id: orgId }))
  }

  async update(id: string, body: Partial<AITool>, orgId: string) {
