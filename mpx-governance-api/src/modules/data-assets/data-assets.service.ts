...
export class DataAssetsService {
  constructor(@InjectRepository(DataAsset) private repo: Repository<DataAsset>) {}
  ...
  async findOne(id: string, orgId: string) {
    ...
    if (!item) throw new NotFoundException(`Data Asset ${id} not found`)
    return item
  }

  create(body: Partial<DataAsset>, orgId: string) { return this.repo.save(this.repo.create({ ...body, organization_id: orgId })) }

  async update(id: string, body: Partial<DataAsset>, orgId: string) {
