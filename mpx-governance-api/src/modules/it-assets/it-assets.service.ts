...
export class ITAssetsService {
  ...
  async findOne(id: string, orgId: string) {
    ...
    if (!item) throw new NotFoundException(`IT Asset ${id} not found`)
    return item
  }

  create(body: Partial<ITAsset>, orgId: string) {
    return this.repo.save(this.repo.create({ ...body, organization_id: orgId }))
  }

  async update(id: string, body: Partial<ITAsset>, orgId: string) {
