...
export class ExternalPartiesService {
  constructor(@InjectRepository(ExternalParty) private repo: Repository<ExternalParty>) {}
  ...
  async findOne(id: string, orgId: string) {
    ...
    if (!item) throw new NotFoundException('ExternalParty ' + id + ' not found')
    return item
  }
  ...
  async create(body: any, orgId: string) {
    ...
    if (!body.code) body.code = 'EP-' + year + '-' + String(count + 1).padStart(4, '0')
