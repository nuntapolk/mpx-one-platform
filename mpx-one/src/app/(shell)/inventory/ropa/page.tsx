...
export default function Page() {
  ...
  const [form, setForm] = useState({ processing_activity_name: "", purpose: "", lawful_basis: "", risk_level: "" })
  ...
  async function create() {
    ...
    setShowForm(false)
    setForm({ processing_activity_name: "", purpose: "", lawful_basis: "", risk_level: "" })
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <Card>
        <SectionHeader title="ROPA — Records of Processing" action={
        ...
        {showForm && (
          <div className="mb-4 p-3 border border-zinc-200 rounded-lg bg-zinc-50 space-y-2">
            <input placeholder="ชื่อกิจกรรม *" value={form.processing_activity_name} onChange={e => setForm(v => ({ ...v, processing_activity_name: e.target.value }))} className="w-full text-xs px-2 py-1.5 border border-zinc-200 rounded focus:outline-none focus:border-[#02C39A]" />
            <input placeholder="วัตถุประสงค์" value={form.purpose} onChange={e => setForm(v => ({ ...v, purpose: e.target.value }))} className="w-full text-xs px-2 py-1.5 border border-zinc-200 rounded focus:outline-none focus:border-[#02C39A]" />
            <input placeholder="Lawful Basis" value={form.lawful_basis} onChange={e => setForm(v => ({ ...v, lawful_basis: e.target.value }))} className="w-full text-xs px-2 py-1.5 border border-zinc-200 rounded focus:outline-none focus:border-[#02C39A]" />
              ...
              <tbody>
                {rows.map((r: any) => (
                  <tr key={r.id} className="hover:bg-zinc-50">
                    ...
                    <Td><span className="text-xs text-zinc-600">{r.lawful_basis || "—"}</span></Td>
                    <Td><span className="text-xs text-zinc-600">{r.data_subject_type || "—"}</span></Td>
                    <Td>{r.cross_border_transfer_flag ? <span className="text-[10px] px-2 py-0.5 rounded-full bg-blue-50 text-blue-700">Yes</span> : <span className="text-zinc-300 text-xs">—</span>}</Td>
                    <Td>{r.dpia_required_flag ? <span className="text-[10px] px-2 py-0.5 rounded-full bg-blue-50 text-blue-700">Yes</span> : <span className="text-zinc-300 text-xs">—</span>}</Td>
